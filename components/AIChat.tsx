...
const AIChat: React.FC = () => {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'model', text: 'Welcome to RHC! Ask me about divisions, registration, or rules. 🏒' }
  ]);
    ...
    <div className="fixed bottom-4 right-4 md:bottom-6 md:right-6 z-50 flex flex-col items-end pointer-events-auto">
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {/* Header */}
            <div className="bg-gradient-to-r from-blue-900/50 to-teal-900/50 p-4 flex justify-between items-center border-b border-white/10">
              <div className="flex items-center gap-2">
                <Sparkles className="w-5 h-5 text-[#4fb7b3] animate-pulse" />
                <h3 className="font-heading font-bold text-white tracking-wider">RHC ASSISTANT</h3>
              </div>
                ...
                >
                  <div
                    className={`max-w-[85%] p-3 rounded-lg text-sm ${
                      msg.role === 'user'
                        ? 'bg-[#4fb7b3] text-black font-medium rounded-tr-none'
            ...
            <div className="p-3 border-t border-white/10 bg-black/40">
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="bg-[#4fb7b3] p-2 rounded-lg hover:bg-[#3da39f] transition-colors disabled:opacity-50"
                  data-hover="true"
                >
      ...
      <motion.button
        ...
        className="w-12 h-12 md:w-14 md:h-14 rounded-full bg-gradient-to-tr from-[#4fb7b3] to-[#637ab9] flex items-center justify-center shadow-lg shadow-[#4fb7b3]/40 border border-white/20 z-50 group"
        data-hover="true"
      >
        {isOpen ? (
